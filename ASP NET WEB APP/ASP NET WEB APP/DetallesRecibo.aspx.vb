...
Public Class DetallesRecibo
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        recibo = CType(Session.Item("recibo"), String)
        ...
        sql = String.Format("select * from detalle,recibo where '{0}' = recibo.idrecibo and recibo.idrecibo = detalle.idrecibo ", recibo)
        ...
        sql = String.Format("select totalfactura,sum(detalle.costo)-totalfactura from recibo,detalle where recibo.idrecibo = {0} and recibo.idRecibo = detalle.idRecibo", recibo)
        ...
        dt.Columns.Add("AV")
        sql = String.Format("select * from refraccionfactura,recibo  where refraccionfactura.idrecibo = recibo.idrecibo and recibo.idrecibo ='{0}' ", recibo)
    ...
    Public Sub FillAbonos()
        ...
        Dim sql As String = Nothing
        Dim connetionString As String = "server=localhost;user id=admin;database=visualdesignsnew;persistsecurityinfo=True;password=admin"
        sql = String.Format("select * from abono where '{0}' = abono.idrecibo ", recibo)
        Dim connection As New MySqlConnection(connetionString)
        connection.Open()
        Dim command As New MySqlCommand(sql, connection)
        adapter.SelectCommand = command
        ...
        'abonos.Columns(3).FooterText = " Descuento"
        ' abonos.Columns(4).FooterText = descuento
        abonos.DataBind()
    ...
    Public Sub loadContact()
        ...
        connectionFooter.Open()
        Dim Sql As String = String.Format("select * from refraccionfactura,recibo  where refraccionfactura.idrecibo = recibo.idrecibo and recibo.idrecibo ='{0}' ", recibo)
        Dim readerReceta As MySqlDataReader
        Dim commandReceta As New MySqlCommand(Sql, connectionFooter)
        readerReceta = commandReceta.ExecuteReader()
    ...
    Public Function loadTitle()
        ...
        Dim sql As String = Nothing
        Dim connetionString As String = "server=localhost;user id=admin;database=visualdesignsnew;persistsecurityinfo=True;password=admin"
        sql = String.Format("select cliente.nombre, cliente.1erapell, cliente.2doapell, recibo.fecha, cliente.telefono from recibo,cliente where recibo.idrecibo='{0}' and recibo.cedula = cliente.cedula", recibo)
        ...
        If reader.Read Then
            ...
            nombreFactura.Text = "     #Recibo:" + recibo + " | " + reader.GetString(0) + " " + reader.GetValue(1).ToString + " " + reader.GetValue(2).ToString + " | " + reader.GetString(3) + "     "
    ...
    Protected Sub printBill(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        Dim temp As String = " -r ""C:\fyi\factura.rdl"" -p ""newIdRecibo=" + recibo + "&newIdFactura=0"""
